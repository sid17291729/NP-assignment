...
int main(){
    ...
    //Server displays received message
        ...
        if(strcmp(msg.payload.header, ":new")==0){
        ...
        else if(strcmp(msg.payload.header, ":new group")==0){           //msg buffer contains the client msg q
            mid_client = msg.payload.client_id; 
            printf("creating new group for %d\n", mid_client);             
            groups[group_count][0] = mid_client;                //setting asking to the group memeber
        ...
        else if(strcmp(msg.payload.header, ":list")==0){
            ...
            for(int i = 0; i<count[msg.payload.gid]; ++i){
                mid_client = groups[msg.payload.gid][i];
